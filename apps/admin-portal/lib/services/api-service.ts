...
export function getErrorMessages(error: unknown): string[] {
  if (error instanceof ApiRequestError) {
    const messages = extractApiValidationMessages(error.body);
    if (messages.length > 0) {
      return messages.map(toUserFriendlyValidationMessage);
    }
  }
  return getErrorMessage(error)
    ...
    .filter(Boolean)
    .map(toUserFriendlyValidationMessage);
}

function extractApiValidationMessages(body: unknown): string[] {
  if (!body || typeof body !== "object") {
    return [];
  }
  ...
  if (payload.error && typeof payload.error === "object") {
    return extractApiValidationMessages(payload.error);
  }
  return [];
}

function toUserFriendlyValidationMessage(message: string): string {
  const dictionary: Record<string, string> = {
    "sellerYears must not be greater than 100":
      "Số năm hoạt động của nhà bán không được lớn hơn 100.",
    "sellerYears must not be less than 0": "Số năm hoạt động của nhà bán không được nhỏ hơn 0.",
    "reviewImageUrls must contain http(s) URLs or absolute public paths":
      "Ảnh đánh giá chỉ được nhập URL http(s) hoặc đường dẫn public bắt đầu bằng dấu /.",
  };
...
export const eligibleCustomerService = {
  async getCustomers(): Promise<EligibleCustomer[]> {
    return (await requestJson<ApiEligibleCustomer[]>("/admin/eligible-customers")).map(
      toEligibleCustomer,
    );
  },
  ...
  async importFile(
    file: File,
  ): Promise<{ imported: number; updated: number; duplicates: number; errors: number }> {
    const form = new FormData();
    form.set("file", file);
    ...
    const result = await requestJson<{
      ...
      invalid: number;
      duplicate: number;
    }>("/admin/eligible-customers/import", { body: form, method: "POST" });
    return {
  ...
  async updateCustomer(
    id: string,
    updates: Partial<EligibleCustomer>,
...
export const integrationService = {
  ...
  async getGoogleSheetConfigs(): Promise<GoogleSheetConfigs> {
    ...
    return {
      eligibleCustomers: configs.eligibleCustomers
        ? toGoogleSheetConfig(configs.eligibleCustomers)
        : null,
...
function toProduct(product: ApiProduct): Product {
  ...
  return {
    ...
    sellerYears: product.sellerYears ?? undefined,
...
function toProductPayload(product: Partial<Product>): Record<string, unknown> {
  return {
    ...
    ...(product.productAttributes !== undefined
      ? { productAttributes: toAttributePayload(product.productAttributes) }
      : {}),
    ...(product.detailImageUrls !== undefined
      ? {
          detailImageUrls: product.detailImageUrls
    ...
    ...(product.sellerYears !== undefined ? { sellerYears: product.sellerYears } : {}),
    ...(product.sellerPrimaryCategory !== undefined
      ? { sellerPrimaryCategory: product.sellerPrimaryCategory }
      : {}),
    ...(product.minimumOrderQuantity !== undefined
      ? { minimumOrderQuantity: product.minimumOrderQuantity }
      : {}),
    ...(product.shippingOrigin !== undefined ? { shippingOrigin: product.shippingOrigin } : {}),
    ...(product.shippingLeadTime !== undefined
      ? { shippingLeadTime: product.shippingLeadTime }
      : {}),
    ...(product.returnPolicy !== undefined ? { returnPolicy: product.returnPolicy } : {}),
    ...(product.reviewRating !== undefined ? { reviewRating: product.reviewRating } : {}),
    ...(product.reviewCount !== undefined ? { reviewCount: product.reviewCount } : {}),
    ...(product.reviewTags !== undefined
      ? { reviewTags: toAttributePayload(product.reviewTags) }
      : {}),
    ...(product.reviewImageUrls !== undefined
      ? {
          reviewImageUrls: product.reviewImageUrls
            .map((imageUrl) => imageUrl.trim())
            .filter(Boolean),
    ...
    ...(product.qualityCertifications !== undefined
      ? { qualityCertifications: toAttributePayload(product.qualityCertifications) }
      : {}),
    ...(product.packagingAttributes !== undefined
      ? { packagingAttributes: toAttributePayload(product.packagingAttributes) }
      : {}),
    ...(product.listedPrice !== undefined ? { listedPrice: product.listedPrice } : {}),
    ...(product.stock !== undefined ? { stockQuantity: product.stock } : {}),
    ...(product.isPromotionEligible !== undefined
      ? { isPromotionEligible: product.isPromotionEligible }
      : {}),
